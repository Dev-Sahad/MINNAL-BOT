...
def _load_trigger_channel_id() -> int:
    try:
        if os.path.exists(SETTINGS_FILE):
            with open(SETTINGS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            return int((data.get("summon_domain") or {}).get("trigger_channel_id") or 0)
    except Exception:
        pass
    return 0
...
class SummonDomain(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.trigger_channel_id = _load_trigger_channel_id()
        self.active_domains = {}
    ...
    def _load_domains(self):
        try:
            if os.path.exists(DOMAINS_FILE):
                ...
                log.info("voice_manager", f"Loaded {len(self.active_domains)} active domains")
        except Exception as e:
            log.error("voice_manager", f"Could not load domains: {e}")
            self.active_domains = {}
    ...
    def _save_domains(self):
        try:
            ...
            log.error("voice_manager", f"Could not save domains: {e}")

    @commands.Cog.listener()
    async def on_voice_state_update(self, member, before, after):
        if member.bot:
            return
        if after.channel and after.channel.id == self.trigger_channel_id and (before.channel is None or before.channel.id != self.trigger_channel_id):
    ...
    async def _create_domain(self, member, trigger_channel):
        try:
            ...
            overwrites = {
                ...
                member: discord.PermissionOverwrite(connect=True, speak=True, manage_channels=True, move_members=True, mute_members=True, deafen_members=True, priority_speaker=True),
                guild.me: discord.PermissionOverwrite(connect=True, manage_channels=True, move_members=True)
            }
            new_channel = await guild.create_voice_channel(name=full_name, category=category, overwrites=overwrites, reason=f"Summon Domain for {member.name}")
            await member.move_to(new_channel)
            self.active_domains[new_channel.id] = {"owner_id": member.id, "guild_id": guild.id}
            self._save_domains()
            log.event("voice_manager", f"Domain created: {full_name}")
            try:
                notif_channel = guild.system_channel
                if notif_channel and notif_channel.permissions_for(guild.me).send_messages:
                    embed = discord.Embed(title="⚡ DOMAIN EXPANSION", description=f"{member.mention} has summoned\n# {domain_name}", color=discord.Color.purple(), timestamp=datetime.now(timezone.utc))
            ...
            log.error("voice_manager", f"Error creating domain: {e}")
    ...
    async def _delete_domain(self, channel):
        try:
            ...
            self._save_domains()
            log.event("voice_manager", f"Domain deleted: {channel.name}")
        except discord.NotFound:
            self.active_domains.pop(channel.id, None)
            self._save_domains()
        except Exception as e:
            log.error("voice_manager", f"Error deleting domain: {e}")
    ...
    async def setdomaintrigger(self, interaction: discord.Interaction, channel: discord.VoiceChannel):
        ...
        try:
            if os.path.exists(SETTINGS_FILE):
                ...
                data = {}
            data.setdefault("summon_domain", {})["trigger_channel_id"] = channel.id
            with open(SETTINGS_FILE, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2)
        except Exception as e:
            log.error("voice_manager", f"Could not persist trigger channel: {e}")
        embed = discord.Embed(title="⚡ Trigger Channel Set", description=f"Trigger: {channel.mention}\n\nUsers joining will summon domains!", color=discord.Color.purple())
        embed.add_field(name="💡 Tip", value="This setting is now saved in data/settings.json.", inline=False)
    ...
    async def domain_rename(self, interaction: discord.Interaction, new_name: str):
        ...
        try:
            ...
            embed = discord.Embed(title="✏️ Domain Renamed", description=f"New name: **{new_name}**", color=discord.Color.green())
            await interaction.response.send_message(embed=embed, ephemeral=True)
        except Exception as e:
    ...
    async def domain_kick(self, interaction: discord.Interaction, member: discord.Member):
        ...
        try:
            ...
            embed = discord.Embed(title="🚫 Domain Banishment", description=f"{member.mention} has been kicked from your domain!", color=discord.Color.red())
    ...
    async def domain_lock(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(title="🔒 Domain Locked", description="Your domain is now locked!", color=discord.Color.dark_purple())
            await interaction.response.send_message(embed=embed)
        except Exception as e:
            await interaction.response.send_message(f"❌ Error: {str(e)}", ephemeral=True)
    ...
    async def domain_unlock(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(title="🔓 Domain Unlocked", description="Your domain is now open!", color=discord.Color.green())
            await interaction.response.send_message(embed=embed)
        except Exception as e:
            await interaction.response.send_message(f"❌ Error: {str(e)}", ephemeral=True)
    ...
    async def domain_info(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="⚡ Domain Information", description=f"**{channel.name}**", color=discord.Color.purple(), timestamp=datetime.now(timezone.utc))
    ...
    async def list_domains(self, interaction: discord.Interaction):
        guild_domains = [(channel_id, data) for channel_id, data in self.active_domains.items() if data["guild_id"] == interaction.guild.id]
        if not guild_domains:
            await interaction.response.send_message("❌ No active domains in this server!", ephemeral=True)
            return
        embed = discord.Embed(title="⚡ Active Domains", description=f"**{len(guild_domains)}** domain(s) currently active", color=discord.Color.purple())
        for channel_id, data in guild_domains[:25]:
            channel = interaction.guild.get_channel(channel_id)
            owner = interaction.guild.get_member(data["owner_id"])
            if channel:
                embed.add_field(name=channel.name, value=f"👑 {owner.mention if owner else 'Unknown'}\n👥 {len(channel.members)} members", inline=True)
        await interaction.response.send_message(embed=embed)
